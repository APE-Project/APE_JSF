var Ape_core = new Class({
	...
	/***
	 * Save session
	 * Set in session some core vars (sessid, user, pipes) 
	 * and fire event save_pipe on each pipe
	 * @param	boolean	Tag session as update	
	 * @param	boolean Make request Async
	 */
	...
	request: function(raw, param, sessid, options){
		if(raw!='CHECK'){
			//remove restore cookie if data is exchanged with server
