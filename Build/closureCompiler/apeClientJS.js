...
a.load=function(b){b=b||{};b.transport=b.transport||APE.Config.transport||0;b.frequency=b.frequency||0;b.domain=b.domain||APE.Config.domain||document.domain;b.scripts=b.scripts||APE.Config.scripts;b.server=b.server||APE.Config.server;b.init=function(g){this.core=g;for(g=0;g<this.eventProxy.length;g++)this.addEvent.apply(this,this.eventProxy[g])}.bind(this);if(b.transport!=2)document.domain=b.domain;var c=this.cookie.read("APE_Cookie"),d=eval("("+c+")");if(d)b.frequency=d.frequency+1;else c='{"frequency":0}';
d=new RegExp('"frequency":([ 0-9]+)',"g");c=c.replace(d,'"frequency":'+b.frequency);this.cookie.write("APE_Cookie",c);var e=document.createElement("iframe");e.setAttribute("id","ape_"+b.identifier);e.style.display="none";e.style.position="absolute";e.style.left="-300px";e.style.top="-300px";document.body.appendChild(e);if(b.transport==2){c=e.contentDocument;if(!c)c=e.contentWindow.document;c.open();d="<html><head></head>";for(var f=0;f<b.scripts.length;f++)d+='<script src="'+b.scripts[f]+'"><\/script>';
d+="<body></body></html>";c.write(d);c.close()}else{e.setAttribute("src","http://"+b.frequency+"."+b.server+'/?[{"cmd":"script","params":{"scripts":["'+b.scripts.join('","')+'"]}}]');e.contentWindow.location.href=e.getAttribute("src")}e.onload=function(){e.contentWindow.APE?e.contentWindow.APE.init(b):setTimeout(e.onload,100)}};if(Function.prototype.bind==null)Function.prototype.bind=function(b,c){return this.create({bind:b,arguments:c})};
