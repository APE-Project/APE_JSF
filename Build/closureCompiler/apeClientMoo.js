...
a.toLowerCase(),b,c)},onError:function(a,b,c){return this.addEvent("error_"+a,b,c)},load:function(a){a=$merge({},APE.Config,a);a.init=function(f){this.core=f;for(f=0;f<this.eventProxy.length;f++)this.addEvent.apply(this,this.eventProxy[f])}.bind(this);if(a.transport!=2)document.domain=a.domain;var b=JSON.decode(Cookie.read("APE_Cookie"),{domain:document.domain});if(b)a.frequency=b.frequency.toInt();else b={frequency:0};b.frequency=a.frequency+1;Cookie.write("APE_Cookie",JSON.encode(b),{domain:document.domain});
var c=(new Element("iframe",{id:"ape_"+a.identifier,styles:{display:"none",position:"absolute",left:-300,top:-300}})).inject(document.body);if(a.transport==2){b=c.contentDocument;if(!b)b=c.contentWindow.document;b.open();for(var d="<html><head>",e=0;e<a.scripts.length;e++)d+='<script src="'+a.scripts[e]+'"><\/script>';d+="</head><body></body></html>";b.write(d);b.close()}else{c.set("src","http://"+a.frequency+"."+a.server+'/?[{"cmd":"script","params":{"scripts":["'+a.scripts.join('","')+'"]}}]');
c.contentWindow.location.href=c.get("src")}c.addEvent("load",function(){c.contentWindow.APE?c.contentWindow.APE.init(a):setTimeout(c.onload,100)})}});
